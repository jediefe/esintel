    ...
    */
    ...
    public function create(CharacterCreateRequest $request) {
        ...
        if ($character->exists()) {
            ...
            return redirect()->route('esintel.view', $character->character_id)->with('success', 'New character entry has been created successfully.');
        }
    }
    ...
    public function editPost(CharacterCreateRequest $request, int $id){
        ...
        $character = Character::where('character_id', $id)->first();
        if (!$character){
            return redirect()->route('esintel.create')->with("categories", Category::getActiveCategories());
        }
        $data = $request->validated();

        if (!($character->character_id == $data['charid'])) {
            return redirect()->back()->withError('Character ID mismatch!');
            }
